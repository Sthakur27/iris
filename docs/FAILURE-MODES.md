...
| 1.5 | Leaning toward the screen during a hard rep | Reduces the actual demand, so the user "succeeds" at a level they cannot really fuse — the single most common way home therapy fools itself. | Distance shown persistently; explicit warning in onboarding. Webcam distance estimation **[defer]**. |
